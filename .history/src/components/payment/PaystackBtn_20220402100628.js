...
function PaystackBtn({ productPrice, className, onSuccess, productID }) {
  ...
  const { getHours, getMinutes, getSeconds, getDate, getMonth, getFullYear } =
    dateTime;
  const hours = getHours().toString;
  const minutes = getMinutes().toString;
  const seconds = getSeconds().toString;
  const monthDate = getDate().toString;
  const month = getMonth().toString;
  const year = getFullYear().toString;
