...
const UploadEmployees = () => {
  ...
  const handleSubmit = (event) => {
    ...
    const employeeRef = ref(storage, `employeesImages/${id}`);
    const uploadTask = uploadBytesResumable(employeeRef, image, metadata);
  ...
  const handleChange = (e) => {
    if (names) {
      ...
      setUploads({ ...uploads, id, type: "employee", timestamp: Date.now() });
    }
